...
// Edit Account
frontend.accountEdit = (data, callback) => {
  // Request any request that isn't a GET
  if (data.method === "get") {
    // Prepare data for interpolation
    const templateData = {
...
frontend.accountDeleted = (data, callback) => {
  // Request any request that isn't a GET
  if (data.method === "get") {
    // Prepare data for interpolation
    const templateData = {
...
// View all the available pizzas
frontend.pizzaList = (data, callback) => {
  // Request any request that isn't a GET
  if (data.method === "get") {
    // Prepare data for interpolation
    const templateData = {
      "head.title": "Menu List",
      "body.class": "pizzaList",
    };

    // Read in template as a string
    helpers.getTemplate("pizzaList", templateData, (err, str) => {
      if (!err && str) {
        helpers.addUniversalTemplates(str, templateData, (err, newStr) => {
          if (!err && newStr) {
            callback(200, newStr, "html");
...
frontend.cartFill = (data, callback) => {
  // Request any request that isn't a GET
  if (data.method === "get") {
    // Prepare data for interpolation
    const templateData = {
      "head.title": "Add item to your cart",
      "body.class": "cartFill",
    };

    // Read in template as a string
    helpers.getTemplate("cartFill", templateData, (err, str) => {
      if (!err && str) {
        helpers.addUniversalTemplates(str, templateData, (err, newStr) => {
          if (!err && newStr) {
            callback(200, newStr, "html");
...
// Add item to your cart
frontend.pizzaOrder = (data, callback) => {
  // Request any request that isn't a GET
  if (data.method === "get") {
    // Prepare data for interpolation
    const templateData = {
      "head.title": "Checkout your Cart",
      "body.class": "pizzaOrder",
    };

    // Read in template as a string
    helpers.getTemplate("pizzaOrder", templateData, (err, str) => {
      if (!err && str) {
        helpers.addUniversalTemplates(str, templateData, (err, newStr) => {
          if (!err && newStr) {
            callback(200, newStr, "html");
