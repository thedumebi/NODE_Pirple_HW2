...
_menu.get = (data, callback) => {
  ...
  const email =
    ...
    RegExp(
      ...
      : false;
  const code = Number(data.queryStringObject.code)
    ? Number(data.queryStringObject.code)
    : false;
  ...
  if (email) {
    ...
    _tokens.verifyToken(token, email, (tokenIsValid) => {
      if (tokenIsValid) {
        if (code) {
          const itemIndex = menuItems.findIndex((x) => x.code === code);
          if (itemIndex > -1) {
            callback(200, menuItems[itemIndex]);
          } else {
            callback(404, { Error: "Requested item does not exist" });
          }
        } else {
